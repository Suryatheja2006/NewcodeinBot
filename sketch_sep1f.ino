...
void updateMotorControl() {
  ...
  if ((gz != 0) && (gx != 0) && (abs(gy) < 2)) {
    ...
    spd = constrain(abs(map((atan2(gx, gz) * 180 / PI), 0, 90, 0, 255)), 0, 255);
    cmd = (gy > 0) ? 6:5 ; // Forw Right or Forw Left
  }else if ((gx <0) && (gy !=0) && (gz !=0)) {
    spd = constrain(abs(map((atan2(gx, gz) * 180 / PI), 0, 90, 0, 255)), 0, 255);
    cmd = (gy > 0) ? 8:7 ; // Back Right or Back Left
...
const int ENB = 12;  // choose the GPIO pin of esp32;
...
void applyMotorControl() {
  ...
  if (spd > 150 && spd < 200) {
    spd = 255;
     analogWrite(ENA, spd);
      analogWrite(ENB, spd/10);
  }
  if (spd > 200 && spd < 255) {
    spd = 255;
    analogWrite(ENA, spd);
      analogWrite(ENB, spd/4);
  }
      break;
    case 6:  // Forw Right
      digitalWrite(IN1, HIGH);
  ...
  if (spd > 150 && spd < 200) {
    spd = 255;
     analogWrite(ENB, spd);
      analogWrite(ENA, spd/10);
  }
  if (spd > 200 && spd < 255) {
    spd = 255;
    analogWrite(ENB, spd);
      analogWrite(ENA, spd/4);
  }
      break;
...
void loop() {
  ...
  delay(0); // Delay to control loop speed
}